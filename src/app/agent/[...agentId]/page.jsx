...
const SingleAgentPage = () => {
  ...
  useEffect(() => {
    ...
    .then(res => res.json())
    .then(data => setAgentData(data.formatedAgent))
  }, [])
  ...
  const { title, slug, agent_meta } = agentData
  return (
    <div>
    <div className='max-w-screen-lg px-6 mx-auto py-4'>
        <AgentHeroSection slug={slug} img={agentData.coverImage.guid.rendered} name={title.rendered} contacts={contacts} languages={languages} address={agent_meta.fave_agent_address} />
      </div>
      ...
      <div className='max-w-screen-lg mx-auto px-6'>
        ...
        <p className='text-lg text-justify pb-4' dangerouslySetInnerHTML={{__html: agentData.content.rendered}}>
        </p>
      </div>

export const data = {
    imageContainer: {
        mainPic: "/property/mainpic.jpg",
        sidePic1: "/property/sidepic1.jpg",
        sidePic2: "/property/sidepic2.jpg",
        imageCarasouel: [
            "/property/mainpic.jpg",
            "/property/sidepic1.jpg",
            "/property/sidepic2.jpg",
        ]
    },
    mainContent : {
        title: "DLF Kings Court",
        sideTitle: "Greater Kailash 2, South Delhi",
        currentStatus: "FOR SALE",
        price : "4,00,00,000",
        about: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Consectetur a erat nam at lectus urna duis. Felis eget velit aliquet sagittis id consectetur purus ut faucibus. Enim ut sem viverra aliquet eget. Facilisis mauris sit amet massa. Ultrices dui sapien eget mi proin sed. Sapien nec sagittis aliquam malesuada bibendum. Suspendisse interdum consectetur libero id faucibus. Vitae semper quis lectus nulla at volutpat diam ut venenatis. Eu sem integer vitae justo eget. Eu scelerisque felis imperdiet proin fermentum leo vel orci porta. Enim praesent elementum facilisis leo. Commodo sed egestas egestas fringilla phasellus faucibus scelerisque eleifend. Leo vel fringilla est ullamcorper eget nulla. Libero volutpat sed cras ornare arcu dui vivamus. Ipsum dolor sit amet consectetur. Ultrices vitae auctor eu augue ut lectus arcu. Urna duis convallis convallis tellus id interdum velit laoreet. Tincidunt augue interdum velit euismod in pellentesque massa placerat duis. Viverra tellus in hac habitasse.",
        overview:{
            projectArea : '2 Acres',
            launchDate : 'Jul, 2011',
            configuration : '5 BHK Apartment',
            sizes : '5500.00 sq.ft - 7500.00 sq.ft',
            avgPrice : '₹47.00 K/sq.ft',
            projectSize: '8 buildings - 54 units',
            possessionStatus: 'Ready to move',
        },
        specification:{
            Parking:{parkingInfo:['Parking : None']},
            Interior:{
                virtualTour: ['Virtual Tour (External Link)'],
                bathroomInfo: ['No. of baths(Full): 1','Bath Amenities : Whirpool'],
                bedroomInfo:['No. of Bedrooms: 2','No. of Bedrooms(possible): 2'],
                room1Info:['Room Type: Bedroom 2','Level: Main','Dimensions: 14x10','Flooring: Hardwood'],
                room2Info:['Room Type: Bedroom 3'],
                room3Info:['Room Type: Bedroom 4'],
                room4Info:['Room Type: Dining Room','Level: Main','Dimensions: 18x10','Flooring: Hardwood'],
                room5Info:['Room Type: Family Room','Dimensions: 8x6','Flooring: Hardwood'],
                room6Info:['Room Type: Living Room','Level: Main','Dimensions: 19x10','Flooring: Hardwood'],
                room7Info:['Room Type: Master Bedroom','Level: Main','Dimensions: 16x12','Flooring: Hardwood'],
                addRoomInfo:['Dining Room: Separate','Basement Description: None','Laundry Features: Gas Dryer Hookup,In Unit, In Kitchen'],
                interiorFeatures:[
                    'Interior Features: Hardwood Floors, Laundry, Hookup In Unit, Builtin Features,Walk-In Closets',
                    'No. of Rooms: 5',
                    'Window Features: Blind(s), Double pane window(s), Screen(s)',
                    'Door Features: Mirrored Closet Door(s), Storm Door(s), 36" Minimum Entry Door, 32" Minimum Interior Door(s)'
                ]
            }
        },
        mapCoordinates:{
            latitude:'28.532728318401112',
            longitude:'77.2406774322555',
        },
        aboutDeveloper :{
            logoSrc:'/property/developerLogo.webp',
            developerInfo:'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Viverra justo nec ultrices dui sapien eget. Varius duis at consectetur lorem donec massa sapien faucibus et. Ante in nibh mauris cursus mattis molestie a iaculis. Amet purus gravida quis blandit turpis cursus. Tortor pretium viverra suspendisse potenti nullam ac tortor. Bibendum at varius vel pharetra. Semper viverra nam libero justo laoreet. Magnis dis parturient montes nascetur ridiculus mus mauris vitae ultricies. Gravida in fermentum et sollicitudin ac. Eleifend donec pretium vulputate sapien nec. Enim nec dui nunc mattis enim. Varius morbi enim nunc faucibus a pellentesque sit. Ut aliquam purus sit amet luctus. Nunc scelerisque viverra mauris in. Scelerisque purus semper eget duis at tellus at. Nec tincidunt praesent semper feugiat. Pulvinar elementum integer enim neque volutpat ac tincidunt vitae.',
        },
        localityOverview :{
            title: 'Greater Kailash 2',
            subtitle: 'Delhi South | Pincode-11048',
            introduction: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Cursus eget nunc scelerisque viverra mauris. Eleifend mi in nulla posuere sollicitudin aliquam ultrices. Tincidunt lobortis feugiat vivamus at augue eget arcu dictum varius. Sodales ut eu sem integer vitae justo eget. Nulla facilisi morbi tempus iaculis urna id volutpat. Morbi tristique senectus et netus. Interdum velit laoreet id donec ultrices tincidunt. Ac tortor dignissim convallis aenean et tortor at risus. Rhoncus dolor purus non enim praesent elementum facilisis leo vel. Ultrices sagittis orci a scelerisque purus semper. Placerat vestibulum lectus mauris ultrices eros in.',
            Pros: ['Ample Supply of ready-to-move-in premium 3BHK and 4BHK apartments.','Served by Greater Kailash Metro Station along the Magenta line of Delhi. The violet Line is nearby too.','N block and M block markets are popular shopping centres and retail centres in South Delhi'],
            Cons: ['Ample Supply of ready-to-move-in premium 3BHK and 4BHK apartments.','Served by Greater Kailash Metro Station along the Magenta line of Delhi. The violet Line is nearby too.','N block and M block markets are popular shopping centres and retail centres in South Delhi'],
        },
        FaqData: [
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
            {Q:'Are there any metro stations near DLF Kings court?',A:'Greater Kailash metro station is near to Dlf Kings Court.'},
        ],
    },
}
...
const ReadMore = ({ isFullScreen, header, subheader, hasCustomHeader }) => {
  ...
  return (
    <div
      className={`${
        isFullScreen
          ? ""
          : "px-4 sm:px-8 md:px-16 lg:px-32 md:space-y-8 md:mt-10 py-2 md:py-5"
      } relative`}
    >
      {!hasCustomHeader && (
        <h2 className="singleDevHeading py-4 flex justify-between gap-2 items-center">
          ...
          {subheader && (
            ...
            <span className="text-[10px]  cursor-pointer leading-normal sm:text-sm text-blue-500 font-bold max-w-[40%]">
              <Link href={"/blog"} >
                {subheader}
              </Link>
              </span>
      ...
      <div className="absolute inset-0 hidden md:block">
        <div
          className={`swiper-nextArrow ${
            curSlide === newsData.length && "swiper-disabledArrow"
          } bg-blue-500 text-white w-[36px] h-[36px]  hidden md:grid items-center justify-center rounded-[100%]  top-[50%] mb-8 -translate-x-1/2 absolute z-10 cursor-pointer hover:bg-blue-500/70 transition-all ${
            isFullScreen ? "right-[-2rem]" : "right-[2rem] lg:right-[6rem] "
          }`}
        >
